...
#define CGI_DLTRIE_ENTRY(type)	struct						\
				{						\
					type *sibling;				\
					type *child;				\
				}
...
#define CGI_DLTRIE_IS_EMPTY(head)	(CGI_DLTRIE_FIRST(head) == CGI_DLTRIE_END(head))

#define CGI_DLTRIE_INIT(elem,field)	((elem)->field.sibling = (elem)->field.child = NULL)
